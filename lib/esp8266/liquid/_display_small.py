...
class Display44780(Device):
 def __init__(self,name,sda=None,scl=None,width=16,height=2,addr=0x27):
  self.present=False
  self.last_text=""
  ...
  try:
   ...
   self.present=True
   Device.__init__(self,name,i2c,setters={"set":self.evaluate},report_change=False)
 ...
 def show(self):
  for line in range(self.char_height):
   for column in range(self.char_width):
    if self.display_buffer[line*self.char_width+column]!=self.back_buffer[line*self.char_width+column]:
     self.dp.move_to(column,line)
     self.dp.putchar(chr(self.back_buffer[line*self.char_width+column]))
     self.display_buffer[line*self.char_width+column]=self.back_buffer[line*self.char_width+column]
  self.dp.move_to(self.x,self.y)
 def scroll(self,show=True):
  for line in range(self.char_height-1):
   self.back_buffer[line*self.char_width:(line+1)*self.char_width]=self.back_buffer[(line+1)*self.char_width:(line+2)*self.char_width]
